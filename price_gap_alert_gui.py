...
class PriceGapAlertGUI:
    ...
    def _build_ui(self) -> None:
        ...
        tk.Label(top, text="제외 티커").grid(row=0, column=4, sticky="w")
        tk.Entry(top, textvariable=self.exclude_var, width=18).grid(row=0, column=5, padx=6)
        ...
        self.once_btn.grid(row=0, column=8, padx=6)
    ...
    def _fetch_and_render(self, symbols: list[str], threshold: float, excluded_symbols: set[str]) -> None:
        ...
        self.log_queue.put(("log", f"[{now}] 공통코인 {len(common_symbols)}개 / 알림대상 {len(alert_symbols)}개 / 제외티커 {len(excluded_symbols)}개"))
    ...
    def run_once(self) -> None:
        ...
        excluded_symbols = core.parse_excluded_symbols(self.exclude_var.get())
        ...
        def task() -> None:
            ...
            try:
                ...
                if not symbols:
                    ...
                    return
                self._fetch_and_render(symbols, threshold, excluded_symbols)
