  ...
  try {
    ...
    const splTokens = tokenAccounts
      .map((item) => {
        ...
        const priceUsd = Number(mintPrices?.[mint] || dexRow?.priceUsd || 0);

        return {
          symbol: dexRow?.symbol || tickerMap.get(mint) || mint.slice(0, 4) + '...' + mint.slice(-4),
          amount,
          priceUsd,
          valueUsd: amount * priceUsd,
        };
  ...
  try {
    ...
    return tokenData
      .map((token) => {
        ...
        const dexRow = dexMeta.get(mint.toLowerCase());
        const symbol =
          token.symbol || token.tokenSymbol || dexRow?.symbol || tickerMap.get(mint) || token.tokenName || (mint ? `${mint.slice(0, 4)}...${mint.slice(-4)}` : 'UNKNOWN');
        const priceUsd = Number(
          token.priceUsd ?? token.price_usdt ?? token.tokenPrice?.usd ?? mintPrices?.[mint] ?? dexRow?.priceUsd ?? 0
        );
