...
async function getSolanaTickerMap() {
  if (!solanaTickerMapPromise) {
    solanaTickerMapPromise = safeFetchJson('https://token.jup.ag/all')
      .then((tokens) => {
        const map = new Map();
...
async function fetchSolanaWallet(address) {
  try {
    ...
    if (mints.length) {
      try {
        const pricePayload = await safeFetchJson(
    ...
    const [solPricePayload] = await Promise.all([
      safeFetchJson('https://api.coingecko.com/api/v3/simple/price?ids=solana&vs_currencies=usd'),
    ]);
    const solPriceUsd = Number(solPricePayload?.solana?.usd || 0);
  ...
  try {
    ...
    return tokenData
      .map((token) => {
        ...
        const priceUsd = Number(token.priceUsd ?? token.price_usdt ?? token.tokenPrice?.usd ?? 0);
