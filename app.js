...
async function getSolanaTickerMap() {
  if (!solanaTickerMapPromise) {
    solanaTickerMapPromise = safeFetchJsonGetWithFallback('https://token.jup.ag/all')
      .then((tokens) => {
        const map = new Map();
...
async function fetchSolanaWallet(address) {
  try {
    ...
    let solscanMetaByMint = new Map();
    try {
      const solscanMeta = await safeFetchJsonGetWithFallback(
        `https://api-v2.solscan.io/v2/account/token-accounts?address=${encodeURIComponent(
          address
        )}&page=1&page_size=200&type=token`
      );
      const rows = solscanMeta?.data?.tokenAccounts || [];
      rows.forEach((token) => {
        const mint = token.tokenAddress || token.mint;
        if (!mint) return;
        solscanMetaByMint.set(mint, {
      ...
      solscanMetaByMint = new Map();
    }

    let mintPrices = {};
    const tickerMap = await getSolanaTickerMap();
    if (mints.length) {
      try {
        const pricePayload = await safeFetchJsonGetWithFallback(
          `https://price.jup.ag/v4/price?ids=${encodeURIComponent(mints.join(','))}`
        );
        mintPrices = pricePayload?.data || {};
      } catch (error) {
        mintPrices = {};
      }
    }
    ...
    const splTokens = tokenAccounts
      .map((item) => {
        ...
        const meta = solscanMetaByMint.get(mint);
        const priceUsd = Number(mintPrices?.[mint]?.price || meta?.priceUsd || 0);

        return {
          symbol: meta?.symbol || tickerMap.get(mint) || mint.slice(0, 4) + '...' + mint.slice(-4),
          amount,
          priceUsd,
          valueUsd: amount * priceUsd,
        };
  ...
  try {
    ...
    return tokenData
      .map((token) => {
        ...
        const mint = token.tokenAddress || token.mint || '';
        const symbol =
          token.symbol || token.tokenSymbol || tickerMap.get(mint) || token.tokenName || mint.slice(0, 4) + '...' + mint.slice(-4) || 'UNKNOWN';
        const priceUsd = Number(
          token.priceUsd ?? token.price_usdt ?? token.tokenPrice?.usd ?? mintPrices?.[mint]?.price ?? 0
        );
...
async function fetchBscTokenBalancesFromTransfers(address) {
  ...
  for (let page = 1; page <= maxPages; page += 1) {
    ...
    const pageRows = parseExplorerRows(payload?.result);
