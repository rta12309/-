...
async function fetchExplorerTokenBalances(chainId, address) {
  ...
  if (!rows.length) {
    const covalentFallback = await fetchCovalentTokenBalances(chainId, address).catch(() => []);
  ...
  if (!rows.length && has404EndpointError) {
    throw new Error('요청 실패 (404): 해당 체인 탐색기 API 경로를 찾지 못했습니다. 스캔 사이트 API 정책 변경/미지원일 수 있습니다.');
  }
