...
async function fetchSolanaWallet(address) {
  try {
    ...
    const mintPrices = await fetchSolanaMintPrices(mints);
    ...
    const splTokens = tokenAccounts
      .map((item) => {
        ...
        const priceUsd = Number(mintPrices?.[mint] || meta?.priceUsd || 0);
    ...
    return tokenData
      .map((token) => {
        ...
        const priceUsd = Number(
          token.priceUsd ?? token.price_usdt ?? token.tokenPrice?.usd ?? mintPrices?.[mint] ?? 0
        );
