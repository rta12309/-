...
3. 각 옵션 오른쪽의 토글(적용 체크박스)로 조건 ON/OFF 조절.
4. `스캔 실행` 클릭.
...
## 웹 입력값 안내 (초보자용)
- **티커**: 예) `USDT`
- **토큰 CA 주소(선택)**: 토큰 컨트랙트 주소(`0x...`). 입력하면 자동 탐색을 건너뜁니다.
...
## 탐색 조건 ON/OFF (웹)
- **USD 보유가치 범위**: 300k~30m 범위를 적용/해제.
- **첫 트랜잭션이 기준시각 이후**: 이 조건이 엄격해서 0건이 나올 수 있으니 필요시 OFF.
...
## “전부 OFF인데도 0건”이 나오는 주요 원인
1. **holder API 자체가 비어 있음**: 체인 explorer 요금제/API 제한 또는 키 미입력 제한.
2. **토큰 holder가 적거나 없음**: 조회 페이지 수/토큰 선택 이슈.
3. **수량 단위 설정 오류**: `Decimals`, `raw` 설정이 실제 토큰 포맷과 불일치.
