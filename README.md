...
- 결과 테이블에 각 코인의 `업비트 입/출금`, `빗썸 입/출금` 상태와 **상태 출처(공식API/더따리크롤링)**를 함께 표시합니다.
- 업비트 입출금 상태 조회는 `api.upbit.com` 실패 시 `sg/id/th` 엔드포인트까지 순차 재시도합니다.
- API에서 상태를 못 가져온 코인은 `https://theddari.com/crypto/<티커>` 경로를 직접 조회(대문자 티커 기준)해서 보조 크롤링합니다.
- 더따리 크롤링은 캐시 + 병렬 요청으로 최적화되어, 자동 탐색 시 재조회 속도를 개선했습니다.
