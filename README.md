...
- 업비트 입출금 상태 확인은 비활성화되어 있습니다.
- 입출금 상태는 빗썸(기존 API)만 조회하고, 업비트는 확인하지 않습니다.
- 업비트 입출금은 `확인안함`으로 표시됩니다.
- 업비트/빗썸 입출금 상태 API가 401/차단될 경우, 해당 경고를 표시하고 **가격 차이 계산은 계속 진행**합니다.
- 조회 중에는 로딩 아이콘(회전 스피너)로 앱이 멈춘 것이 아닌지 바로 확인할 수 있습니다.
- 가격차 감지 알림은 브라우저 차단 팝업(`alert`) 대신 오른쪽 아래 토스트로 표시되어 탐색이 멈추지 않습니다.
- GitHub Pages에 이 파일을 올리면 웹 링크로 바로 실행할 수 있습니다.

## CORS용 업비트 상태 비활성화 서버 (Flask)

브라우저는 업비트 API를 직접 호출하지 않고 서버(`/api/*`)만 호출합니다.
서버는 업비트 입출금 상태 기능을 비활성화한 안내 응답만 반환합니다.

### 파일

- `app.py`: Flask 서버 (CORS + 업비트 상태 비활성화 API)
- `upbit_wallet_status_client.html`: 업비트 상태 비활성화 안내 UI
...
- `GET /api/upbit_wallet_status?symbols=BTC,ETH,XRP`
  - 업비트 상태 비활성화 안내 JSON 반환
- `GET /debug/upbit_raw`
  - 비활성화 상태 안내 반환

### 보안/설정

- 업비트 API 키/JWT/공지 파싱을 더 이상 사용하지 않습니다.
