...
def parse_excluded_symbols(raw: str) -> set[str]:
...
def fetch_transfer_statuses_safe(symbols: Sequence[str]) -> Tuple[Optional[Dict[str, TransferStatus]], Optional[Dict[str, TransferStatus]], List[str]]:
    ...
    warnings.append("업비트 입출금 확인은 비활성화되어 표시하지 않습니다.")
...
def filter_restricted_coins(
    ...
    tradable: List[str] = list(symbols)
...
def render_restricted_coins(restricted: Sequence[RestrictedCoin], limit: int = 20) -> str:
    ...
    lines = [f"🚫 입출금 제한 코인 {len(restricted)}개 (참고 표시, 알림은 계속)"]
...
def monitor(threshold: float, interval: int, once: bool, excluded_symbols: set[str]) -> int:
    try:
    ...
    while True:
        ...
        try:
            ...
            alert_symbols = [s for s in tradable_symbols if s not in excluded_symbols]

            for warning in status_warnings:
                print(f"[{now}] 경고: {warning}", file=sys.stderr)

            filtered_upbit = {s: upbit_prices[s] for s in alert_symbols if s in upbit_prices}
            filtered_bithumb = {s: bithumb_prices[s] for s in alert_symbols if s in bithumb_prices}

            alerts = find_gap_alerts(filtered_upbit, filtered_bithumb, threshold)
            print(f"[{now}] 공통코인 {len(common_symbols)}개 / 알림대상 {len(alert_symbols)}개 / 제외티커 {len(excluded_symbols)}개")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="알림에서 제외할 티커 목록(쉼표 구분, 예: BTC,ETH)",
    )
...
def main() -> int:
    ...
    if args.interval <= 0:
        ...
        return 2
    excluded_symbols = parse_excluded_symbols(args.exclude)
    return monitor(threshold=args.threshold, interval=args.interval, once=args.once, excluded_symbols=excluded_symbols)
